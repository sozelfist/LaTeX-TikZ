...
\begin{document}
    \begin{tikzpicture}
        ...
        \foreach \y in {0, 1, 2, 3, 4, 5}{
            ...
            % Hidden layer 1
            \node[hl1] (hl1-\y) at (2.5, \y) {};
            % Hidden layer 2
            \node[hl2] (hl2-\y) at (5, \y) {};
            % Hidden layer 3
            \node[hl3] (hl3-\y) at (7.5, \y) {};
        }
        \foreach \y in {2, 3}{
            % Output layer
            \node[op, pin={[pin edge={-latex, gold}, pin distance=1.5cm]right:$ $}] (op-\y) at (10, \y) {};
        }
        ...
        \foreach \pos/\layer/\label/\name/\col in {
            ...
            \node[anchor=south, text width=2cm, text centered, inner sep=0pt] (\label) at (\pos.north) {\color{\col}\tiny$\mathsf{\layer}$ \\[-2pt] \name};
        }
        \node[anchor=south, text width=2cm, text centered, inner sep=0pt, yshift=0.75cm] at (op-3.north) {\tiny$\mathsf{\textcolor{gold}{T_4}=\textcolor{green}{\hat{Y}}}$ \\[-2pt] \textcolor{gold}{Output Layer}};
        \node[text width=2cm, text centered, inner sep=0pt, xshift=-1.5cm, color=blue!50] (feature/image) [left of=inp] {\tiny$\mathsf{X}$ \\[-2pt] Feature/Image};
        \node[text width=2cm, text centered, inner sep=0pt, xshift=-1.5cm, color=green] (label) [left of=feature/image] {\tiny$\mathsf{Y}$ \\[-2pt] Label};
        % add names of image
        \node[anchor=center, color=green] at (-5, 3.5) {Cat};
        \node[anchor=center, color=green] at (-5, 1.5) {Dog};
        % add images
        \node[anchor=center] at (-2.5, 3.5){\includegraphics[scale=0.05]{./images/cat.jpg}};
        \node[anchor=center] at (-2.5, 1.5) {\includegraphics[scale=0.135]{./images/dog.jpg}};
    \end{tikzpicture}
\end{document}